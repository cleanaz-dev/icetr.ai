...
import { checkFeature } from "@/lib/services/tier-service";
...
export async function POST(req, { params }) {
  ...
  try {
    ...
    // 1. Look up the key first (no self-reference)
    const apiKeyRecord = await prisma.apiKey.findFirst({
      where: {
        plainKey: apiKeyHeader,
        isActive: true,
        expiresAt: { gte: new Date() }, // safe now
      },
      select: {
        id: true,
        campaignIds: true,
    ...
    // 2. Pull tier settings (correct model name)
    const tierSettings = await prisma.tierSettings.findUnique({
      where: { orgId: apiKeyRecord.orgId },
      select: { tier: true, features: true },
    });

    // 3. Feature check
    const webhookEnabled = await checkFeature(
      tierSettings,
      "webhookIntegrationsEnabled",
      { action: "send-via-api" }
    );

    if (!webhookEnabled.allowed) {
      return NextResponse.json(
        { error: "Webhook integrations are not enabled for your plan" },
        { status: 403 }
      );
    }
    ...
    if (!firstName || !lastName || !phoneNumber) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: firstName, lastName, and phoneNumber are required",
        },
        { status: 400 }
      );
    }
    ...
    await prisma.leadActivity.create({
      data: {
        leadId: newLead.id,
        type: "NOTE",
        content: `Lead created via API. Source: ${
          additionalData.source || "API"
        }`,

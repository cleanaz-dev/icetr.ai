  ...
  try {
    ...
    if (data.type === "call") {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (!call) {
          console.warn(
            `No active call found for leadId: ${id} and sessionId: ${data.sessionId}. This might indicate the call wasn't properly created in TwiML route.`
          );
          return NextResponse.json(
            {
              error:
                "Active call record not found. Ensure call was initiated properly.",
            },
            { status: 404 }
          );
        }
          ...
          if (existingAttempts) {
            ...
            activity = await tx.leadActivity.update({
              ...
              data: {
                ...
                callSessionId: data.sessionId,
                content: `${
                  existingAttempts.callAttemptCount + 1
                } contact attempts - Latest: ${data.outcome}`,
                outcome: mappedOutcome,
                duration:
                  (existingAttempts.duration || 0) + (data.duration || 0),
        ...
        const followUpOutcomes = ["SCHEDULED_CALLBACK", "CALLBACK"];
        ...
        console.log(
          `Call completed successfully: ${call.callSid}, Outcome: ${mappedOutcome}`
        );
      });

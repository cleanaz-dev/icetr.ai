...
export async function POST(request, {params}) {
  ...
  // Get phone configuration for this organization
  const phoneConfig = await getPhoneConfig(orgId);
  console.log("Phone config loaded:", { 
    recordingEnabled: phoneConfig.recordingEnabled,
    transcriptionProvider: phoneConfig.transcriptionProvider,
    inboundFlow: phoneConfig.inboundFlow 
  });
  ...
  try {
    ...
    if (recordingUrl && callSid && recordingStatus === "completed") {
      ...
      try {
        ...
        if (wasTrainingCall) {
          ...
          // Always transcribe training calls for feedback (ignore config)
          const transcription = await handleTranscription(recordingUrl, transcriptionText, 
            { ...phoneConfig, transcribeInbound: true, transcriptionProvider: 'assemblyai' }, 'inbound');
        ...
        if (followUp || prospect) {
          ...
          // Check if inbound recording is enabled
          if (phoneConfig.recordInboundCalls) {
            ...
            if (followUp) {
              ...
              await updateFollowUpWithRecording(callSid, recordingUrl, transcription);
            } else if (prospect) {
              console.log("Updating prospect with recording for unknown caller");
              await updateProspectWithRecording(callSid, recordingUrl, transcription);
            }
          } else {
          ...
          // Check if outbound recording is enabled and meets duration requirements
          if (phoneConfig.recordOutboundCalls && phoneConfig.recordingEnabled && 
            ...
            await updateCallWithRecording(callSid, recordingUrl, transcription, duration);
          } else {
    ...
    if (leadId && callSessionId && callSid) {
      try {
        const call = await createOrUpdateCall(callSid, leadId, callSessionId, from, to, direction, callStatus, userId);
        if (call) {
    ...
    // Client-initiated call (outbound through client)
    ...
    // Real inbound call
      ...
      // Handle lead/prospect creation based on config
      ...
      // Handle call flow based on configuration
    ...
    // Outbound calls initiated by application
    else if (direction === "outbound-api" || (leadId && callSessionId)) {
      const targetNumber = to;

      if (targetNumber) {
        console.log(`Outbound call to ${targetNumber} - recording: ${phoneConfig.recordingEnabled && phoneConfig.recordOutboundCalls}`);

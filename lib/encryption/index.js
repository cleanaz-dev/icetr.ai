...
function deriveOrgKey(organizationId, keyVersion = '1') {
  const masterKey = getMasterKey();
  const salt = crypto.createHash('sha512')
    .update(`${organizationId}|${keyVersion}|${process.env.ENCRYPTION_SALT || ''}`)
    .digest();
  ...
  return crypto.hkdfSync(
    ...
    masterKey,
    salt,
    `org-key-${organizationId}-v${keyVersion}`,
    KEY_LENGTH
  );
}

export function encryptIntegrationData(data, organizationId, keyVersion = '1') {
  try {
    if (!data || typeof data !== 'object') {
      throw new EncryptionError('Invalid data format', 'INVALID_DATA');
    }

    const orgKey = deriveOrgKey(organizationId, keyVersion);
    ...
    // Add context to AAD
    cipher.setAAD(Buffer.from(`${organizationId}:${ENCRYPTION_VERSION}`));
...
export function decryptIntegrationData(encryptedData, organizationId) {
  try {
    const [version, keyVersion, ivHex, tagHex, dataHex] = encryptedData.split(':');
    ...
    decipher.setAAD(Buffer.from(`${organizationId}:${version}`));
...
export async function rotateOrgKeys(organizationId, currentVersion = '1') {
  // Implementation note:
...
export function healthCheck(organizationId) {
  const testData = { test: Date.now() };
  try {
    const encrypted = encryptIntegrationData(testData, organizationId);
    const decrypted = decryptIntegrationData(encrypted, organizationId);
    return {

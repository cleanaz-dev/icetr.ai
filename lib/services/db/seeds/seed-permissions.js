...
async function syncPermissions() {
  ...
  const toCreate = permissions.filter(p => !existingPermissionNames.has(p.name));
  const unused = existingPermissions.filter(p => !requiredPermissionNames.has(p.name));
  
  // Create new permissions
  if (toCreate.length > 0) {
    await Promise.all(
      toCreate.map(permission => prisma.permission.create({ data: permission }))
    );
    console.log(`✅ Created ${toCreate.length} new permissions: ${toCreate.map(p => p.name).join(', ')}`);
  }
  
  // Warn about unused permissions (don't delete them)
  if (unused.length > 0) {
    console.log(`⚠️  Found ${unused.length} permissions in DB not defined in file: ${unused.map(p => p.name).join(', ')}`);
    console.log(`   These permissions are kept but won't be assigned to any roles.`);
  }
...
async function syncRoles(permissionMap) {
  ...
  for (const roleData of roles) {
    ...
    if (!role) {
      ...
      console.log(`✅ Created new role "${roleData.type}" with ${requiredPermissions.length} permissions`);
    } else {
      ...
      const hasChanges = currentPermissionIDs.size !== newPermissionIDs.size ||
      ...
      if (hasChanges) {
        ...
        const added = requiredPermissionIDs.filter(id => !currentPermissionIDs.has(id));
        const removed = [...currentPermissionIDs].filter(id => !newPermissionIDs.has(id));
        
        console.log(`🔄 Updated role "${roleData.type}":${added.length > 0 ? ` +${added.length} permissions` : ''}${removed.length > 0 ? ` -${removed.length} permissions` : ''}`);
      } else {
  ...
  for (const permission of allPermissions) {
    const validRoleIDsForPermission = permission.roleIDs.filter(roleId => validRoleIDs.has(roleId));
    ...
    if (JSON.stringify(validRoleIDsForPermission.sort()) !== JSON.stringify(shouldHaveRoleIDs.sort())) {
      await prisma.permission.update({
        where: { id: permission.id },
        data: {
...
async function seedPermissionsAndRoles() {
  try {
    ...
    console.log(`📊 Final state: ${finalPermissions.length} permissions, ${finalRoles.length} roles`);

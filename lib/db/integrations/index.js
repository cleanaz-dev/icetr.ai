...
/**
 * Retrieves the Hapio configuration for a given organization.
 *
 * @async
 * @function getAiAgentConfig
 * @param {string} orgId - The ID of the organization to fetch the Hapio configuration for.
 * @returns {Promise<{
 *   resource: any,
 *   location: any,
 *   service: any,
 ...
 *   scheduleBlocks: any
 * } | null>} The Hapio configuration object if found, otherwise `null`.
 *
 * @example
 * const hapioConfig = await getHapioConfig("org_123");
 * console.log(hapioConfig?.resource);
 */
export async function getAiAgentConfig(orgId) {
  const query = await prisma.organization.findUnique({
    where: { id: orgId },
    select: {
      aiAgentSettings: {
        select: {
          agentIstructions:true,
          webhookUrl: true,
          resource: true,
          location: true,
          service: true,

...
export async function roleBasedLeadsMulti(orgId, campaignId, teamId) {
  ...
  if (!assignedTo) {
    throw new Error(
      "No team member with role LEAD found in the specified team."
    );
  }
...
export async function assignNextInLine({ teamId, orgId, campaignId, leadId }) {
  const team = await prisma.team.findFirst({
    where: { id: teamId, orgId },
    select: {
      members: {
  ...
  const redisKey = `lead_assign_pointer:${teamId}:${campaignId}`;
  let currentIndex = parseInt(await redis.get(redisKey)) || 0;
  const assignedUser = members[currentIndex];
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.leadActivity.create({
      data: {
        ...
        type: "ASSIGNMENT",
        content: `Lead assigned to ${assignedUser.fullname}`,
        createdUser: { connect: { id: assignedUser.id } },
      },
    });
    ...
    await tx.notification.create({
      data: {
        ...
        type: "LEAD_ASSIGNED_FROM_FORM",
        title: "Lead Assigned",
        message: `New Lead has been assigned.`,
      },
    });
  });

  currentIndex = (currentIndex + 1) % members.length;
  await redis.set(redisKey, currentIndex);
  return { assignedUser };
}

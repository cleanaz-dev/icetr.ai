...
/**
 * Fetch a training scenario by its unique ID.
 *
 * @async
 * @function getTrainingScenarioById
 * @param {string} trainingScenarioId - The unique identifier of the training scenario.
 * @returns {Promise<Object|null>} The training scenario object if found, otherwise `null`.
 */
export async function getTrainingScenarioById(trainingScenarioId) {
  const query = await prisma.trainingScenario.findFirst({
    where: { id: trainingScenarioId },
  })
  return query
}

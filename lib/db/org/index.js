...
/**
 * Retrieves the organization details that a specific user belongs to.
 *
 * @param {string} userId - The Clerk user ID to match against organization users.
 * @param {string} orgId - The ID of the organization to retrieve.
 * @returns {Promise<Object|null>} The organization object with related data if found, otherwise null.
 *
 * @example
 * const organization = await getUserOrganization("user_123", "org_456");
 * if (organization) {
 *   console.log(organization.tierSettings);
 * }
 */
export async function getUserOrganization(userId, orgId) {
...
export async function getOrgRecentActivity(orgId) {
...
export async function getOrgBillingData(clerkId, orgId) {
  const billingData = await prisma.organization.findUnique({
    where: {
      id: orgId,
      users: {
        some: {
  ...
  return { 
    customerData:billingData.customer,
    invoices: billingData.customer.invoices,
    subscription: billingData.customer.subscription
  }
}


export async function getOrgPhoneNumbers(orgId) {
  const query = await prisma.organization.findUnique({
    where: { id: orgId },
    select: {phoneNumbers: true },
  })
  return query.phoneNumbers || []
}
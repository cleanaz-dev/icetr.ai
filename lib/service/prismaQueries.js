...
export async function getTeamTrainingData(teamId, userId) {
  // 1. check caller role
...
if (!["Admin", "Manager"].includes(caller.role)) {
  ...
  // 2. return every training for the team
  const teamTrainings = await prisma.training.findMany({
    ...
    include: {
      user: {
        select: { id: true, firstname: true, lastname: true, imageUrl: true, role: true },
      },
    },
  });

  return {teamTrainings:teamTrainings}
}
...
export async function getTeamAndMembers(userId) {
  // First, get the user with their teamId
  const user = await prisma.user.findUnique({
    where: { clerkId: userId },
    select: {
      teamId: true,
  ...
  // Get team with members, campaigns, and manager
  const teamWithMembersAndCampaigns = await prisma.team.findUnique({
    where: {
      id: user.teamId,
    },
    select: {
  ...
  // Handle case where team is not found (shouldn't happen if data is consistent)
  if (!teamWithMembersAndCampaigns) {
    throw new Error("Team not found");
  }

  return {
    team: teamWithMembersAndCampaigns,
    members: teamWithMembersAndCampaigns.members,
    campaigns: teamWithMembersAndCampaigns.campaigns,
  };
}
...
export async function getAllOrgLeads(userId) {
  try {
    ...
    const leads = await prisma.lead.findMany({
      where: { organizationId: userOrgId.orgId },
...
export async function getTeamLeads(userId) {
  // First, get the user with their team information
  const userWithTeam = await prisma.user.findUnique({
    where: { clerkId: userId },
    include: {
      team: {
        include: {
          members: true, // Include all team members
        },
      },
    },
  });
  ...
  // Extract all team member IDs (including the current user)
  const teamMemberIds = userWithTeam.team.members.map((member) => member.id);

  // Now fetch all leads assigned to any team member
  const teamLeads = await prisma.lead.findMany({
    where: {
      assignedUserId: {
        in: teamMemberIds, // Leads assigned to any team member
      },
    },
    include: {
